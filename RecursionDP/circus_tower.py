...
def circus_tower(people):
    ...
    # Time O(N^2), Space O(N), where N is the num of people.
    ...
    def circus_tower_helper(people, bottom, offset, tower_map):
        ...
        if bottom == None or newBottom[1] > bottom[1]:
            if not tower_map[offset]:
                tower_map[offset] = circus_tower_helper(
                    people, newBottom, offset + 1, tower_map)
                tower_map[offset] += 1
            numberWithBottom = tower_map[offset]
        numberWithOutBottom = circus_tower_helper(
            people, bottom, offset + 1, tower_map)
